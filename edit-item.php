...
<?php
  ...
  $select_itemdata=mysqli_query($con,"select * from shopitem where itemid='$itemid' ");
  $fetch_data=mysqli_fetch_array($select_itemdata);
  ...
  $item_color=$fetch_data['itemcolor'];
...
<div id="form-body" style="width:90%;background: white;
  ...
  <form onsubmit="submit_data(); return false; " >
    ...
    height: 115px;">
     ...
     <label for="product-cover-image" style="width:80%;height:80px;margin-top:0px;">
        ...
        <center>  <img src="photos/<?php echo $itemimg; ?>" id="main-img" style="object-fit: contain;width:77px;height:77px;"></center>
          <span class="material-icons" id="cover-img-icon"style="font-size:40px;display:none;">image</span>
        </div>
      </label>
  ...
  <div id="add-new-image-body">
    ...
    <?php
    $select_itemimg=mysqli_query($con,"select * from item_images where itemid='$itemid' ");
    //$fetch_data=mysqli_fetch_array($select_itemdata);

    while($data=mysqli_fetch_array($select_itemimg)) {
  ...
  <br>
    ...
    <input type="number" id="product-price" name="product-price" min="0" placeholder="Product Price" value="<?php echo $item_price; ?>" step="0.01"  required>
